...
//    [self storeImageToCache];
    ...
    TestOpenCVUIViewController *finalView = [[TestOpenCVUIViewController alloc] init];
    
    UIImage *temp = [[UIImage alloc] initWithCGImage: _sourceImage.CGImage
                                                scale: 1.0
                                          orientation: UIImageOrientationLeft];
    
    
    finalView.sourceImage = _sourceImage;
...
- (void)storeImageToCache
{
    ...
    NSString *tmpPath = [NSString stringWithFormat:@"%@/%@", [NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES) objectAtIndex:0], @"test.jpg"];
    NSData* imageData = UIImageJPEGRepresentation(_adjustedImage, 1.0);
