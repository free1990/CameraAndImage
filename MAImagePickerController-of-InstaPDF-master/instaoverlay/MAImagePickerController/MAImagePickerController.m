...
- (void)viewDidLoad
{
    ...
    if (_sourceType == MAImagePickerControllerSourceTypeCamera && [UIImagePickerController isSourceTypeAvailable:UIImagePickerControllerSourceTypeCamera])
    {
        [[NSNotificationCenter defaultCenter] addObserver:self
                                                 selector:@selector(MAImagePickerChosen:)
                                                     name:@"MAIPCSuccessInternal" object:nil];
        ...
        CGRect layerRect = CGRectMake(30, 25, self.view.bounds.size.width-60, self.view.bounds.size.height - kCameraToolBarHeight -50);
...
//        [gridCameraView setFrame:CGRectMake(0, 0, self.view.bounds.size.width, self.view.bounds.size.height - kCameraToolBarHeight)];
        gridCameraView.contentMode = UIViewContentModeScaleAspectFit;
        [gridCameraView setFrame:layerRect];
...
- (void)addTargetMark:(CGRect)rect{
    

    UIBezierPath *path = [[UIBezierPath alloc] init];

    [path moveToPoint:CGPointMake(rect.origin.x, rect.origin.y+20)];
    [path addLineToPoint:rect.origin];
    [path moveToPoint:rect.origin];
    [path addLineToPoint:CGPointMake(rect.origin.x+20, rect.origin.y)];
    
    [path moveToPoint:CGPointMake(rect.origin.x + rect.size.width - 20, rect.origin.y)];
    ...
    [path addLineToPoint:CGPointMake(rect.origin.x + rect.size.width , rect.origin.y+20)];
    
    [path moveToPoint:CGPointMake(rect.origin.x + rect.size.width, rect.origin.y+rect.size.height -20)];
    ...
    [path addLineToPoint:CGPointMake(rect.origin.x + rect.size.width - 20, rect.origin.y+rect.size.height)];
    
    [path moveToPoint:CGPointMake(rect.origin.x + 20, rect.origin.y+rect.size.height)];
    ...
    [path addLineToPoint:CGPointMake(rect.origin.x, rect.origin.y + rect.size.height - 20)];
